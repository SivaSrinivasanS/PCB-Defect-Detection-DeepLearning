...
RUN apt-get update && apt-get install -y \
    curl \
@ -15,19 +15,31 @@ RUN apt-get update && apt-get install -y \
    libxrender1 \
    git \
...
# This will ensure the model is present in the deployed environment.
...
# Copy the rest of your application code (including src/pcb_ui.py)
COPY . .
